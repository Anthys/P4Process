ArrayList<PVector> make_cool_curve(float x,float y,float l,int n, String type){
    ...
    PVector uvb = uv.add( new PVector(1.,1.));
    ...
    for (int i=0;i<n;i++){
        ...
        if (type == "dir"){
            ...
            n_pos = pos.add(dir.mult(partial_l));
        }
        else if (type == "r"){
            float aa =2;
...
void plot_cool_curve(PGraphics img, ArrayList<PVector> P, boolean debug){    
    float var1 = 0;
    color col = colpoint(P.get(-1).x+var1, P.get(-1).y+var1);
    ...
    for (int tt=0;tt<n;tt++){
        ...
        PVector p = spline_4p(float(tt)/n, P.get(j-1), P.get(j), P.get(j+1), P.get(j+2));
        float s = ns-ns*q;
        img.strokeWeight(s);
        img.point(p.x,p.y);
    ...
    var1 = 20;
    color col2 = colpoint(P.get(-1).x+var1, P.get(-1).y+var1);
    img.stroke(col2);
...
PVector spline_4p(float  t, PVector p_1, PVector p0, PVector p1, PVector p2 ){
  PVector a = p_1.mult(t*((2-t)*t - 1));
  PVector b = p0.mult((t*t*(3*t - 5) + 2));
  PVector c = p1.mult(t*((4 - 3*t)*t + 1));
  PVector d = p2.mult(((t-1)*t*t));
...
color colpoint(float x, float y){
    ...
    float varc = 10;
    color col = color(noise(uv.x*varc, varc*uv.y, 25)*255, 30,noise(uv.x*varc, varc*uv.y, 75)*255,20);
    return col;
}
