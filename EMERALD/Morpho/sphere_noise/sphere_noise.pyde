...
def alternate_shape(s, x0=0,y0=0, z0=0, sx = 1, sy=1, sz=1, ss= 1, rx=0, ry=0, rz=0):
    ...
    for i in range(s.getChildCount()):
        ...
        fill(c)
        if i < 255:
            fill(i%255,50.,float(c))
        elif i < 255*2:
            fill(255-i%255,50.,float(c))
        for j in range(f.getVertexCount()):
            v = f.getVertex(j)
            mg = v.mag()
            dir = copy.deepcopy(v).normalize()
            tm = 1./3
            v = v.add(dir.mult(cos(t/20.)/3+noise((v.x+5*v.z)*tm,v.y*tm,t/50.)))
            if rx:
                v = protateX(v,rx)
