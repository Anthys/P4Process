...
class Particle{
  ...
  void update(){
    ...
    for (int i = 0;i<ndir+1;i++){
      ...
      cdir.normalize();
      float da = collision(cdir, npoints, ldist);
    ...
    if (countcol >0){
      ...
      int npoints_side = 10;
      float dis = 60;
      for (int k=0; k<npoints_side+1;k++){
        float cdis= .5*dis/npoints_side*k;
        cvs2.stroke(c, 50);
        cvs2.beginShape();
        for (int j = 0;j<rempos.size()-1;j++){
          PVector cp = rempos.get(j);
          PVector cpn = rempos.get(j+1);
          PVector dir = PVector.sub(cpn, cp);
          PVector normale = new PVector(-dir.y, dir.x);
          normale.normalize();
          cp.add(PVector.mult(normale, cdis));
    ...
    pos.add(vel);
    if (timer%300==1 || timer==0||timer==1){rempos.add(pos.copy());};
    timer += 1;
  }
  
  float collision(PVector dir, int npoints, float dist){
    float delta = npoints;
