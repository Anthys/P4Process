...
void main() {
  ...
  float a = 0.2*(0.5+0.5*snoise(vec2(theta*d, t)));

  final_coord.xy +=vec2(a,a);


  gl_FragColor = texture2D(texture, final_coord.st) * final_col;
  ...
  final_col.rgb = vec3(dist/2,0.,0.);
  final_coord.xy += final;
