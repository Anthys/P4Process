...
def pos_paper(t):
    return PVector(width -float(width/2+100*cos(t)), height-float(height/2+100*sin(t)))
...
def draw():
    ...
    for i in range(n):
        ...
        #c = color(c)
        pg.fill(c)
        postx = width -float(width/2+100*cos(t*speed-.2-s/80.))
    ...
    image(pg, 0,0)
    resetShader()
    paperplane(posp.x, height-posp.y,30, rot=-(t*speed)+PI/2)
    t+=1.
