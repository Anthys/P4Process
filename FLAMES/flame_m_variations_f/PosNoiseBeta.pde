...
class PosNoiseB {
    ...
    void update(){
      if (state == 0){
        ...
        if (n_iterations % 50 == 51){
          render();
          image(final_cvs, 0,0);
      };
        if (n_iterations >= 500*int(pow(4, 1))){
          render();
          
          PGraphics image1 = createGraphics(width, height);
          image1.beginDraw();
          cback = 0;
          image1.background(cback);
          image1.endDraw();
          //stroke(35, 190, 25, 255);
          //rect(0, 0, width, height);
          //fill(45, 50, 29);
          //circle(50, 50, 100);
          image1.blend(final_cvs, 0, 0, width, height, 0, 0, width, height, BLEND); 
          //final_cvs.blend(image1, 0, 0, width, height, 0, 0, width, height, ADD); 
          image1.save("out2_vbig2.png");
          //saveFrame("out_####.png");
          state = 1;
        }
      }
