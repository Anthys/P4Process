...
void setup(){
  size(1000, 1000);
    cvs_orig = loadImage("b.jpg");
  cvs = createImage(cvs_orig.width, cvs_orig.height, RGB);
  //cvs_orig.filter(BLUR, 4);
  //colorMode(RGB, mx_color);
  strokeWeight(20);
  surface.setSize(cvs_orig.width, cvs_orig.height);
}
int i=0;
float mx_color = 1.;
PImage cvs_orig;
...
void theloop(){
  ...
  //for (int i=0; i<width*2+height*2; i++){
    ...
    sortRow(line_colors);
    draw_line(cvs_orig,line_colors,  int(width/2), int(height/2), x, y);
...
int getFirstNotWhite(ArrayList<Integer> lc, int i) {
  while(lc.get(i) > blackValue) {
    i++;
    if(i >= lc.size()) 
      return -1;
...
int getNextWhite(ArrayList<Integer> lc, int i) {
