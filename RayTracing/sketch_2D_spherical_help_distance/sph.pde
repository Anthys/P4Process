...
static float sph_dist2(float theta1, float phi1, float theta2, float phi2){
  // theta 0 2PI
  // phi 0 PI
  return acos(sin(theta1)*sin(theta2)+cos(theta1)*cos(theta2)*cos(phi2-phi1));
}
static float sph_dist3(float theta1, float phi1, float theta2, float phi2){
  // theta 0 2PI
  // phi 0 PI
  return 2*asin(sqrt(pow(sin((theta2-theta1)/2), 2)+cos(theta1)*cos(theta2)*pow(sin((phi2-phi1)/2), 2)));
}
static class sphVec{
  float theta = 0; // 0 2PI
  float phi = 0; // 0 PI
