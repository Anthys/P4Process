...
def make_wave_3D(inivec, vectors, t, speed=1./50, ss=3, ampli=1,n_cycle = 1, omega=10, puis=1, start = PVector(0,0,0)):
    r = t*speed
    one_cycle = PI
    for i,vv in enumerate(vectors):
        d = vv.mag()
        iniv = inivec[i]
        if  r-d>0 and r-d < one_cycle:
            vv.y = iniv.y+ampli*sin(d*omega - t*speed)**puis
    #replace_v("plana.obj", cur_vec)
    return cur_vec
