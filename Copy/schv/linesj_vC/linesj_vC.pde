...
void draw_(){
  ...
  float rc = min(width/2, height/2)-100 + random(-100,100);
  ...
  int n_lines_c = 500;
  for (int i=0;i<n_lines_c;i++){
    float theta = random(0,TWO_PI);
    
    float xf = rc*cos(theta);
    float yf = rc*sin(theta);
    
    float l_line = 50;
    
    float theta2= theta + noise(theta)/2;
    float x0 = (rc-l_line)*cos(theta);
    float y0 = (rc-l_line)*sin(theta);
    
    color col = find_color(theta+2);
