...
/*
 ...
 // SEPARATE RANDOM DONE ONLY FOR COLUMNS, I HAVE TO DO IT FOR ROWS
...
void setup() {
  ...
  cp5.addSlider("sliderRandChanceCol_e", 0,1, .9, 100, 260+100*6, 100*2, 14*3).setId(7);
  ...
  cp5.addSlider("sliderminpathLengthColumns", 0,400, 0, 100, 260+100*4, 100*2, 14*3).setId(11);
  ...
  if(theEvent.isFrom(r1)) {
    ...
    switch(theEvent.getId()) {
      ...
      case(6):
        ...
        break;
      case(7):
        rand_chance_columns_end = theEvent.getController().getValue();
        break;
      case(8):
      ...
      case(9):
        ...
        break;
      case(10):
        rand_chance_columns_start = theEvent.getController().getValue();
        break;
      case(11):
        min_path_length_columns = (int)theEvent.getController().getValue();
        break;
    }
  }
  //re_draw();
...
int getFirstPixelWithConditionY(int mode, int x, int y){
  ...
  while(!checkCond(mode, c) || random(1)<rand_chance_columns_start){
    y++;
    if(y >= img.height) 
      return -1;
    c = get_col(img, x, y,noise_on_columns);
  }
  letruc=(int)(400-colordist(c,colortarget))/100*min_path_length_columns;
  return y;
}
...
int getNextPixelWithConditionY(int mode, int x, int y){
  ...
  while( (checkCond(mode, c) || random(1)<rand_chance_columns_end) || (pathLength<letruc) ) {
    y++;
    pathLength++;
    if(y >= img.height) 
        return img.height-1;
