...
void rayTrace(sphVec p){
  ...
  for (int i = 0; i<nrays;i++){
    ...
    PVector pos_plane = project(pos_cart);
    PVector end_plane = project(probe);
    point(pos_plane.x, pos_plane.y);
    line(pos_plane.x, pos_plane.y, end_plane.x, end_plane.y);
...
PVector project(PVector p){
  return new PVector(p.x/(1-p.z), p.y/(1-p.z));
}
