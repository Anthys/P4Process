...
int[] make_rotate(){
  ...
  int newHeight = (int) Math.floor(h * c + w * s);
...
void draw() {
  ...
  cvs.beginDraw();
  //cvs.translate(-(sin(angle_rays)*(img.height-sin(angle_rays)*wh[0])),(sin(angle_rays)*(img.width-sin(angle_rays)*wh[1]))+sin(angle_rays)*wh[1]*cos(PI/2-angle_rays));
  cvs.rotate(-angle_rays);
  cvs.translate(-sin(angle_rays)*wh[1],0);
  cvs.image(img, 0, 0, img.width, img.height);
  //cvs.image(img, -(sin(angle_rays)*(img.height-sin(angle_rays)*wh[0])), 0, img.width, img.height);
  cvs.endDraw();
  image(cvs, 0,0);
