...
float patternG( in vec2 p, out vec2 q, out vec2 r )
{
    q.x = fbm1( p + vec2(0.0,0.0) );
    q.y = fbm1( p + vec2(5.2,1.3) );

    r.x = fbm1( p + 4.0*q + vec2(1.7,9.2) );
    r.y = fbm1( p + 4.0*q + vec2(8.3,2.8) );
...
void main() {
  ...
  vec2 q = vec2(1.8,2.3);
  vec2 r = vec2(5.2,4.3);
	float a = patternG(st,q,r);
	color = vec3(a*.5+.5);
  color = vec3(r.y*0.5+0.5,q.x*r.y*0.5+.5,r.x*0.5+.5);
