...
class cool_curve{
  float a1, a2, res, med_r, max_s, speed;
  color c1, c2, c3;
...
void draw(){
  ...
  for (cool_curve c:curves){
    c.a1 += c.speed;
    c.a2 += c.speed;
    my_curve(c.a1, c.a2, c.c1, c.c2, c.c3, c.res, c.med_r, c.max_s);
  }
}
...
void draw_(){
  ...
  for (int i = 0; i<n;i++){
    ...
    curves.add(new cool_curve(a1, a2, c1, c3, c2, res, med_r, max_s));
  }
